...
for name in table_names_list:
    query2 = "SELECT row_to_json(n_alias) from (select * from {}) n_alias;".format(name)
    cursor.execute(query2)
    file_content = cursor.fetchall()
    data = json.dumps([row[0] for row in file_content]).encode('UTF-8')
    s3_client.put_object(Body=data, Bucket='dvdrentalbucket', Key='{}.json'.format(name))
